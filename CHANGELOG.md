...
- fix(Header): replace `AppHeaderDropdown` with `UncontrolledDropdown`
...
- refactor(Header): move to ReactRouter `NavLink`
...
- refactor(Dashboard): code splitting with `React.lazy` Aside, Footer, Header, routes 
...
- fix(Dashboard): `AppAside` remove deprecated `hidden` prop 
